...
namespace kvdb {
    ...
    void Cli::prompt() {
        ...
        kvdb::Status status = kvdb::Status::SERROR;
        ...
        if(cmd == SELECTDB) {
            ...
            if(!db->open()) {
                PRINT_ERROR("%s", "failed to open database");
                prompt();
                return;
            }
            status = kvdb::Status::SOK;
            PRINT("database '%s' selected", db->name.c_str());
        } else if(cmd == Action::APUT || cmd == Action::AGET || cmd == Action::ADELETE) {
            if(db == nullptr || !db->opened) {
                PRINT_ERROR("No database selected. Please open a database first.\nTry: selectdb /path/your_db", nullptr);
                prompt();
                return;
            }
            ...
            if(table_action != nullptr) {
                ...
                if(it == db->tables.end()) {
                    ...
                    if(!t->open()) {
                        PRINT_ERROR("failed to create table %s", table_action->table_name.c_str());
                        prompt();
                        return;
                    }
        ...
        if(status == kvdb::Status::SOK) {
            PRINT("Ok %s", duration_str.c_str());
        } else {
            PRINT_ERROR("%s", "failed to process the query");
        }
        prompt();
    }
