...
namespace kvdb {
    ...
    Stream::Stream(const std::string &path, const char *mode) {
        ...
        this->mode = mode;
        if(this->path.empty()) {
            PRINT_ERROR("keys_stream path empty", nullptr);
            return;
        }
        if(!file_exists(this->path.c_str()) || file_ptr == nullptr) {
            file_ptr = fopen(this->path.c_str(), mode);
            if(file_ptr == nullptr || !seek(0)) {
                return;
            }
        }
        if(is_write_mode() && !seek_end()) {
    ...
    bool Stream::close() {
        ...
        PRINT_ERROR("failed to close keys_stream %s\n%s", path.c_str(), strerror(errno));
        return false;
    }
    ...
    bool Stream::is_write_mode() const {
        return strcmp(mode, O_WRITE) == 0;
    }

    bool Stream::create_file(const char *path) {
        return fopen(path, O_WRITE) != nullptr;
    }
